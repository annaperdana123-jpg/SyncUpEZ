...
describe('Kudos Repository', () => {
  const testTenantId = 'test-tenant';
  const testFromEmployeeId = 'emp-001';
  const testToEmployeeId = 'emp-002';
  const testKudos = {
    id: 'uuid-123',
    tenant_id: testTenantId,
    from_employee_id: testFromEmployeeId,
    to_employee_id: testToEmployeeId,
  ...
  describe('getKudos', () => {
    test('should retrieve kudos with pagination', async () => {
      const mockData = [testKudos];
      const mockCount = 1;
      ...
      const result = await kudosRepository.getKudos(testTenantId, 1, 10);
    ...
    test('should throw error when Supabase returns error', async () => {
      ...
      await expect(kudosRepository.getKudos(testTenantId, 1, 10))
        .rejects
        .toThrow(errorMessage);
    });
  });

  describe('getKudosByEmployeeId', () => {
    test('should retrieve kudos by employee ID', async () => {
      // Mock the Supabase chain following the exact pattern from employeeRepository.test.js getEmployeeById
      const eq2Mock = jest.fn().mockResolvedValueOnce({ data: [testKudos], error: null });
      ...
      const result = await kudosRepository.getKudosByEmployeeId(testTenantId, testToEmployeeId);
      
      expect(result).toEqual([testKudos]);
      
      // Verify the Supabase calls were made correctly
      expect(fromMock).toHaveBeenCalledWith('kudos');
      expect(eq1Mock).toHaveBeenCalledWith('tenant_id', testTenantId);
      expect(eq2Mock).toHaveBeenCalledWith('to_employee_id', testToEmployeeId);
    ...
    test('should throw error when Supabase returns error', async () => {
      ...
      await expect(kudosRepository.getKudosByEmployeeId(testTenantId, testToEmployeeId))
        .rejects
        .toThrow(errorMessage);
    });
  });
  ...
  describe('createKudos', () => {
    test('should create new kudos', async () => {
      const kudosData = { ...testKudos };
      delete kudosData.id;
      delete kudosData.tenant_id;
      
      // Mock the Supabase chain following the pattern from employeeRepository.test.js
      const selectMock = jest.fn().mockResolvedValueOnce({ data: [testKudos], error: null });
      const insertMock = jest.fn().mockReturnValue({ select: selectMock });
      const fromMock = jest.fn().mockReturnValue({ insert: insertMock });
      
      require('../src/utils/supabaseClient').from.mockImplementation(fromMock);

      const result = await kudosRepository.createKudos(testTenantId, kudosData);
    ...
    test('should throw error when Supabase returns error', async () => {
      ...
      await expect(kudosRepository.createKudos(testTenantId, kudosData))
        .rejects
        .toThrow(errorMessage);
    });
  });
